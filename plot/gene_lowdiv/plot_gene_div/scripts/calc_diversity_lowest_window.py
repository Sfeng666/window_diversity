...
description = '''Function: calculate window nucleotide diversity for regions(windows) of interest'''
version = '%prog 11.26.2021'
...
parser.add_option("--out_window_stats",
                    ...
                    help = "output path of window stats of all selected windows for all samples analyzed (.txt)",
                    metavar = "PATH")
parser.add_option("--out_window_diversity",
                    action="store",
                    dest = "out_window_diversity",
                    help = "output path of window nucleotide diversity of selected windows (.txt)",
                    metavar = "PATH")
...
parser.add_option("--win_len",
                    ...
                    metavar = "INT")
parser.add_option("--win_coord",
                    action="store",
                    dest = "win_coord",
                    help = "input parameter of window coordinates of the selected window",
                    metavar = "STR")                    
...
win_start = int(win_coord.split(',')[0])
win_end = int(win_coord.split(',')[1])
...
NR==FNR {D[$1$2]++; next} \
!($1$2 in D) && NR>FNR && $2>=''' + str(win_start) + ''' && $2<=''' + str(win_end) + ''' {print}' \
<(cat ''' + in_indel_pos + ''') \
<(zcat ''' + in_mpileup + ''') \
> ''' + indelfree_mpileup + '''
python2.7 ''' + script + ''' \
...
if len(windows) > 0:
    ...
    with open(in_snp, 'r') as f:
        for line in f:
            if not line.startswith('#'):
                ...
                if pos < win_start or pos > win_end:
                    continue
                num_snp += 1
    ...
    header = ["contig", "start_coord", "window"] + samples
    print_header = True
    ...
    with open(out_window_stats, 'a') as f, open(out_window_diversity, 'a') as f_div:
        ...
        for win in windows:
            ...
            f_div.write('\t'.join([contig, str(windows[win]['pos'][0]), str(win)] + list(str(x) for x in nt_win)) + '\n')
