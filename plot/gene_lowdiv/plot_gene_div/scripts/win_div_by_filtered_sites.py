...
parser.add_option("--win_len", dest="win_len", help="input parameter of the numebr of filtered sites that should be contained in each window")
...
parser.add_option("--win_coord", dest="win_coord", help="input parameter of the coordinates of the selected window")
...
for line in load_data(data):    
    ...
    k = line[:-1].split('\t')
    CHR,POS,REF = k[:3]
    if int(POS) < win_start or int(POS) > win_end:
        continue
